...
import torch  #GPU-CUDA #pip install --pre torch --index-url https://download.pytorch.org/whl/nightly/cu128
...
supported_extensions = (".m4a", ".mp3", ".mp4", ".wav", ".avi", ".vob",
                        ".mov", ".mkv", ".flac", ".ogg", ".webm", ".flv", 
                        ".rmvb", ".wmv")

# ---------------------- Tkinter初始化 ----------------------
root = tk.Tk()
root.title("WhisperGUI视频-语音文件识别字幕并生成str")
root.geometry("900x600")  # 整体窗口尺寸设置为900x600
...
export_format_var = tk.StringVar(root, value="SRT")  # 导出格式选项，SRT 或 TXT
...
def select_files():
    ...
    filenames = filedialog.askopenfilenames(
        title="选择音视频文件",
        filetypes=[("Media Files", "*.m4a *.mp3 *.mp4 *.wav *.avi *.mov *.mkv *.flac *.ogg *.webm"), 
                   ("All Files", "*.*")])
    if filenames:
        for f in filenames:
...
def process_files_func():
    ...
    for file in selected_files:
        ...
        # 根据导出格式决定文件扩展名（txt或srt）
        if export_format_var.get() == "TXT":
            output_filename = f"{name}.{suffix}.txt" if suffix else f"{name}.txt"
        else:
...
def start_recognition():
    if not selected_files:
        log("请先选择音视频文件或文件夹！")
        return
    # 新增判断：如果保存模式为“统一存放到指定文件夹”，但未选择输出文件夹，则不启动识别
